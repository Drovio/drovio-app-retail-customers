...
$viewContainer = HTML::select(".customerFinances .viewContainer")->item(0);
...
$infoRow = geFinancesInfoRow("balance", number_format($balance, 2)." €");
DOM::append($viewContainer, $infoRow);

// Invoice container
$customerFinances = HTML::select(".customerFinances")->item(0);
$invoiceContainer = DOM::create("div", "", "", "invoiceContainer");
DOM::append($customerFinances, $invoiceContainer);

// Add action to edit button
$editButton = HTML::select(".customerFinances .edit")->item(0);
$attr = array();
$attr['cid'] = $customerID;
$actionFactory->setAction($editButton, $viewName = "customers/editCustomerFinancialInfo", $holder = ".customerFinancesContainer .editFormContainer", $attr, $loading = TRUE);
